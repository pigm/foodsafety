...
namespace TrazabilidadCarne.Services
{
    ...
    public class InventarioCarnes : IInventarioCarnes
    {
        ...
        /// <returns></returns>
        public ResultadoProceso ProcesaBarrasImgTablet(decimal item, decimal local, decimal procedencia, decimal origenFrigorifico, decimal certificadoEmbarque, string fechaFaena, decimal pesaNeto, decimal pesaBruto, string responsable, byte[] image)
        {
            var img = image;
            Datos.Repositorio.RepositorioBalanza repositorioBalanza = new Datos.Repositorio.RepositorioBalanza();
            decimal itemVenta = item;
            ...
            repositorioBalanza.CargaBarrasImg(itemPadre, local, procedencia, origenFrigorifico, certificadoEmbarque, fechaFaena, pesaNeto, pesaBruto, "0", responsable,img);
            return new ResultadoProceso { EstadoResutado = 0, Item = itemPadre.OLD_NBR };
        }
    }
}
